package com.betterthannoobs.codeutsava.hackathon.blooddonorapp.utils.constants;

public class HttpRequestConstants {


    public static final String REQUEST_REGISTER = "user/register";
    public static final String REQUEST_LOGIN = "account/login";
    public static final String REQUEST_BECOME_DONOR = "become_donor/";
    //    public static String BASE_URL = "http://172.22.124.196:8000/";
    public static String BASE_URL = "http://172.22.124.196:8000/";
    public static final String REQUEST_BLOOD_BANKS = "bloodbank_list/";
    public static final String REQUEST_DONOR_LIST = "donor/list";

    private HttpRequestConstants() {
    }


}
